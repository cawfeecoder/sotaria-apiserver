...
// SotariaServer contains state for a Kubernetes cluster master/api server.
type SotariaServer struct {
...
// New returns a new instance of SotariaServer from the given config.
func (c completedConfig) New() (*SotariaServer, error) {
	kubeCleintConfig, err := restclient.InClusterConfig()
	if err != nil {
		return nil, err
	}
	kubeClient, err := kubernetes.NewForConfig(kubeCleintConfig)
	if err != nil {
		return nil, err
	}
